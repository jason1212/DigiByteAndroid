...
public final class BitcoinIntegration
{
	...
	{
		final StringBuilder uri = new StringBuilder("digibyte:");
		if (address != null)
			uri.append(address);
		if (amount != null)
	...
	{
		Toast.makeText(context, "No Digibyte application found.\nPlease install Digibyte Wallet.", Toast.LENGTH_LONG).show();

		final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=de.schildbach.wallet.digibyte"));
		final Intent binaryIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://github.com/HashEngineering/digibyte-wallet/releases"));
